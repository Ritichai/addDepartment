...
("zoom_out_map", "e56b")
`
const modifiedDataIcon = dataIcon.replace(/\((.*?)\)/g, "{$1}"); //เปลี่ยน ( ) เป็น { }
export const icons = modifiedDataIcon.trim().split('\n').map((icon, index) => {
  const match = icon.match(/\w+/g);
  const icon_name = match?.[0];
  const icon_code = match?.[1];
  return {
    id: index + 1,
    icon_name,
    icon_code
